...
export const AuthProvider = ({ children }) => {
  ...
  const [authLoading, setAuthLoading] = useState(true); // State loading để chờ kiểm tra auth ban đầu
  const [logoutLoading, setLogoutLoading] = useState(false); // State loading cho hành động logout
  ...
  const checkAuthStatus = useCallback(async () => {
    ...
    try {
      ...
      try {
        console.log("[AuthContext] /users/me ->", user);
      } catch (_) {}
      setCurrentUser(user);
    } catch (error) {
      // Nếu có lỗi (ví dụ: 401 Unauthorized), nghĩa là chưa đăng nhập
      try {
        console.warn("[AuthContext] /users/me error ->", error);
      } catch (_) {}
      setCurrentUser(null);
    } finally {
  ...
  const logout = async () => {
    ...
    try {
      ...
      try {
        console.log("[AuthContext] logout -> clearing user");
      } catch (_) {}
      setCurrentUser(null);
      await checkAuthStatus(); // ✅ confirm lại bằng cookie
